...
%% 
function [invadeTimeNumber,invadeTime,laneDataNumber] = ...
    CalculatePET_All(vehicleData,invadeTimeNumberIn,invadeTimeIn,laneDataNumberIn,stepTime,lane)
    
    global PETSum PETSumNumber allStepVehiclePETNumber
    ...
    lastVehicleNumer = 0;
    for i = 1 : 10
        lastVehicleNumer = lastVehicleNumer + allStepVehiclePETNumber(lane,stepTime-i);
        if(stepTime-i == 0)
           break; 
        end
    end
    ...
    for vdi = 1 : vehicleDataNum
        %计算每一车辆越过侵入线的时刻
        [id, stepTime, vehicleinvadeTime] = CalculateInvadeTime(vehicleData(vdi,2),vehicleData(vdi,3),vehicleData(vdi,4),vehicleData(vdi,5));
        invadeTimeTemp = [id, stepTime, vehicleinvadeTime];
        %标记，用于记录车辆是否重复出现
        labelI = 1;
        ...
        elseif laneDataNumberIn > 0
            ...
            if laneDataNumberIn >1 && labelI == 1
                ...
                %保存PET数据
                PETTemp = [stepTime, vehicleId1, vehicleId2, vehiclePET];
                PETSum(PETSumNumber,:) = PETTemp;
                PETSumNumber = PETSumNumber + 1;
            end
        end
    end
