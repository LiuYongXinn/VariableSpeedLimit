...
%%
function ReinforceLearningForLaneCloseByTTC_2248(vissimFileName,vissimPathName,vissimUnifiedName,...
...
for tti = 1 : trainingNum
    ...
    while step <= evaluationTime
        ...
        if(step >= 300 && step < 600 && mod(step,60) == 0)
            ...
            %车辆按车道和位置进行分类
            [vehicleDataLane1, vehicleDataLane2] = ...
                AgentSortByLaneAndLinkCoordOnlineForTwoLane_2248(vehicleData);
            ConvertInvadeLineAndTTCSum(1, invadeLineSet);
            TTCTimes = TTCTimesCalculate_All(step, vehicleDataLane1, vehicleDataLane2);
            TTCTimesSum1 = TTCTimesSum1 + TTCTimes;
            ConvertInvadeLineAndTTCSum(2, invadeLineSet);
