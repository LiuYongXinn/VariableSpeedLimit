...
% 
%%
function ReinforceLearningByPET_2248(vissimFileName,vissimPathName,vissimUnifiedName,...
   ...
   %设置随机种子
    ...
    while step <= evaluationTime
        ...
        if(step >= 300 && step < 600 && mod(step,60) == 0)
            ...
            PETTimes = PETTimesCalculate_All(step, vehicleDataLane1, vehicleDataLane2, vehicleDataLane3, vehicleDataLane4);
            PETTimesSum1 = PETTimesSum1 + PETTimes;
            ConvertInvadeTimeForSceneAndUpstream_All(2,invadeLineSet);
        end
    ...
    if(mod(tti, 2000) == 0)
        ...
        tempDirection = strcat(vissimPathName,"训练次数\QtableByPET",num2str(tti),".txt");
