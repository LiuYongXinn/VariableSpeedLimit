...
% 
%%
function ReinforceLearningByTTC_2248(vissimFileName,vissimPathName,vissimUnifiedName,...
...
discountFactor = 0.8;
%动作集
[limitSet,motionSet] = AgentActionDefine("confluence");
% 根据不同的天气选择不同的Q表
...
for tti = 1 : trainingNum
    ...
    %设置匝道车流量
    vehicleInput2 = netVehicleInputs.GetVehicleInputByNumber(2);
    ...
    while step <= evaluationTime
        ...
        if(step >= 300 && step < 600 && mod(step,60) == 0)
            ...
            %车辆按车道和位置进行分类
            [vehicleDataLane1, vehicleDataLane2, vehicleDataLane3, vehicleDataLane4] = ...
                AgentSortByLaneAndLinkCoordOnline_2248(vehicleData);
            ConvertInvadeLineAndTTCSum(1, invadeLineSet);
            TTCTimes = TTCTimesCalculate_All(step, vehicleDataLane1, vehicleDataLane2, vehicleDataLane3, vehicleDataLane4);
            TTCTimesSum1 = TTCTimesSum1 + TTCTimes;
            ConvertInvadeLineAndTTCSum(2, invadeLineSet);
        ...
        if(step == 600)
            ...
            %匝道流量设置
            vehicleInput2.set('AttValue', 'Volume',volumeLimit);
        ...
        if (step == 1200)
            ...
            vehicleInput2.set('AttValue', 'Volume',0);
        end
        step = step + 1;
    end
    ...
    if(mod(tti, 2000) == 0)
        ...
        tempDirection = strcat(vissimPathName,"训练次数\QtableByTTC",num2str(tti),".txt");
