...
% 
%%
function ReinforceLearningByDensity_2248(vissimFileName,vissimPathName,vissimUnifiedName,...
...
% 根据不同的天气选择不同的Q表
[Qtable,Qtabledirection] = AgentQtableChoose_2248(vissimPathName,userWeatherChooseRL, 3);
volume1Temp = 300;
%% 主循环
for tti = 1 : trainingNum
    ...
    while step <= evaluationTime
        ...
        if (step == 1200)
            ...
            %根据事故风险设置reward
            reward = AgentRewardByEfficiency(state2);
            disp(reward);
            %更新Q表
    ...
    if(mod(tti, 2000) == 0)
        ...
        tempDirection = strcat(vissimPathName,"训练次数\", "QtableByDensity",num2str(tti),".txt");
